...
import { Form } from '../../05/06/Form';
import { clickSubmit, getGroupByName } from "../../05/07/testingUtils";
import { deliveryAddresses } from "../../05/06/fixtures";
...
describe('過去のお届け先がある場合', () => {
  ...
  test('「はい」を選択、入力、送信すると、入力内容が送信される', async () => {
    ...
    const inputValues = await inputContactNumber();
    const inputDeliver = await inputDeliveryAddress();
    await clickSubmit();
    expect(mockFn).toHaveBeenCalledWith(expect.objectContaining({...inputValues, ...inputDeliver}))
  })
});
